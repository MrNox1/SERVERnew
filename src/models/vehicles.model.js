...
async function addNewVehicles(obj) {
  ...
  try {
    ...
    result = await pool
      ...
      .input("imagen", mssql.VarChar, image).query(`
...
async function selectAllVehicles() {
  ...
  try {
    ...
    console.error("Error al seleccionar vehículos:", err.message);
  } finally {
    if (pool) {
      pool.close();
    }
...
async function selectVehiclesById(obj) {
  ...
  try {
    pool = await connection();
    result = await pool.request().input("myid", mssql.Int, id).query(`
          SELECT 
          v.id AS idVeiculo,
    ...
    console.error("Error al agregar el vieculo:", err.message);
  } finally {
    if (pool) {
      pool.close();
    }
...
async function updateVehicles(obj) {
  ...
  try {
    ...
    result = await pool
      ...
      .input("id", mssql.Int, id)
      .query(
        "UPDATE vehiculos SET nombre = @nombre OUTPUT INSERTED.* WHERE id = @id"
      );
